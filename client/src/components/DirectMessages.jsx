...
import { Send, Lock, Unlock, ShieldCheck, User, Circle, Zap, Repeat, ArrowLeft, Paperclip, X, Loader2, Image as ImageIcon, FileText } from 'lucide-react';
import {
  importPublicKey,
  deriveSharedAESKey,
...
export default function DirectMessages({ currentUser, allUsers, serverUrl, wsClient }) {
  ...
  // Sequence Map for Double Ratchet KDF chain rolling: peerName -> current sequence
  const [ratchetSeqMap, setRatchetSeqMap] = useState({});

  // Derive Shared Pairwise AES Key whenever selected peer changes
  useEffect(() => {
    async function setupPairwiseKey() {
      ...
      try {
        ...
        console.error('Failed to derive pairwise shared key:', err);
      }
    }
  ...
  // Load chat history between currentUser & selectedPeer
  ...
  // Decrypt chat messages and attached media with Double Ratchet Per-Message KDF Chain
  useEffect(() => {
    ...
    async function decryptAllMessages() {
      ...
      for (const m of messages) {
        ...
        if (!msgMeta) {
          ...
          let textContent = isLegacyExpired ? 'Message from an earlier session' : decryptedRaw;
          let mediaId = null;
  ...
  // Listen to live WebSocket direct message events
  useEffect(() => {
    if (!wsClient) return;
  ...
  // Send E2EE Direct Message with Double Ratchet Per-Message Key Rolling (supports text, attachment, or both)
  const handleSendMessage = async (e) => {
    ...
    const sharedKey = sharedKeyMap[selectedPeer.username];
    if (!sharedKey) {
      alert('Cryptographic handshake not complete yet.');
      return;
    }
    ...
    return (
      <div className="dm-contacts-screen">
        <div className="dm-contacts-header">
          <User size={20} />
          <h2>Directory &amp; Contacts</h2>
        </div>

        {peers.length === 0 ? (
          <div className="dm-contacts-empty">
            <Lock size={40} color="#94a3b8" />
            <p>No other registered users online.</p>
            <span>Open another browser tab to simulate Bob or Charlie!</span>
          </div>
        ) : (
            ...
            {peers.map(peer => (
              ...
              >
                ...
                <div className="contact-info">
                  ...
                  <div className="contact-key-snippet">
                    SPKI: {peer.publicIdentityKey.slice(0, 16)}...
                  </div>
                </div>
    ...
    <div className="dm-chat-screen">
      ...
      <div className="chat-header">
        ...
        <div className="peer-profile">
          ...
          <div>
            <h4>Chat with {selectedPeer.username}</h4>
            <span className="handshake-status">
              <Zap size={14} color="#f59e0b" />
              Double Ratchet Active (Post-Compromise &amp; Forward Secrecy)
            </span>
          </div>
        </div>

        <div className="key-details-badge">
          <span>Ratchet KDF: <code>HMAC-SHA256</code></span>
        </div>
      </div>
      ...
      <div className="messages-log">
        {messages.length === 0 ? (
          <div className="empty-chat">
            ...
            <p>Start a secure, Double-Ratcheted conversation with {selectedPeer.username}.</p>
            <span>Every message advances the KDF key chain so past &amp; future messages and attached files remain completely secure.</span>
          </div>
        ) : (
          messages.map(msg => {
              ...
              <div key={msg.id} className={`message-bubble-row ${isMine ? 'mine' : 'peer'}`}>
                <div className="message-bubble">
                  ...
                  <div className="msg-meta">
                    {msg.ratchetSeq ? (
                      <span className="ratchet-badge" title="Double Ratchet Per-Message Key Rolling">
                        <Repeat size={10} />
                        N={msg.ratchetSeq}
                      </span>
                    ) : (
                      <span className="ratchet-badge legacy" title="Static ECDH Session Key">
                        <ShieldCheck size={10} />
                        Static ECDH
                      </span>
                    )}
      ...
      <form onSubmit={handleSendMessage} className="chat-input-form">
        ...
        <input
          type="text"
          placeholder={attachedMedia ? 'Add a caption (optional)...' : `Type Double-Ratcheted message to ${selectedPeer.username}...`}
          value={inputMessage}
